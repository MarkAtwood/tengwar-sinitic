#!/usr/bin/env python3
"""
Render Xiang (Changsha) Tengwar samples to SVG.

Usage:
    python render.py                      # render all samples
    python render.py xiang-common-phrases # render specific sample
"""
...
def render_text_to_svg(lines_data, output_path, font_size=48):
    """
    Render lines with xiang romanization, hanzi, tengwar, and names to SVG.
    ...
    svg = [
        ...
        '<defs><style>',
        f'.romanization {{ font-family: sans-serif; font-size: {int(font_size*0.5)}px; fill: #666; }}',
    ...
    for romanization, hanzi, tengwar, names in lines_data:
        svg.append(f'<text x="20" y="{y}" class="romanization">{romanization}</text>')
        y += row_height
...
def render_sample(name):
    ...
    from tengwar_xiang import convert_text
    from generate_samples import tengwar_to_names
    ...
    # Parse markdown tables and re-generate tengwar from romanization
    lines_data = []
...
def main():
    ...
    samples = ['xiang-common-phrases', 'xiang-proverbs']
    ...
    print("Rendering Xiang (Changsha) Tengwar samples...")
