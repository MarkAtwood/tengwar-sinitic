#!/usr/bin/env python3
"""
Render Hakka Tengwar samples to SVG.
...
def render_text_to_svg(lines_data, output_path, font_size=48):
    """
    Render lines with hakka romanization, hanzi, tengwar, and romanized names to SVG.

    Args:
        lines_data: list of (hakka, hanzi, tengwar, names) tuples
    ...
    svg = [
        ...
        '<defs><style>',
        f'.hakka {{ font-family: sans-serif; font-size: {int(font_size*0.5)}px; fill: #666; }}',
    ...
    y = 30
    for hakka, hanzi, tengwar, names in lines_data:
        svg.append(f'<text x="20" y="{y}" class="hakka">{hakka}</text>')
        y += row_height
...
def render_sample(name):
    ...
    # Re-generate tengwar from hakka romanization for accurate rendering
    lines_data = []
    ...
    for line in content.split('\n'):
        if '|' in line and not line.startswith('|--'):
            ...
            if len(parts) >= 3:
                ...
                hanzi = parts[2]
                # Skip header rows
                if hakka and hanzi and 'Hakka' not in hakka and hakka != '':
                    try:
                        tengwar = convert_text(hakka)
                        names = tengwar_to_names(tengwar)
                        lines_data.append((hakka, hanzi, tengwar, names))
                    except Exception as e:
                        print(f"  Warning: could not convert '{hakka}': {e}")
...
def main():
    # Available samples
    samples = ['tones', 'numbers', 'greetings', 'places']
    ...
    print("Rendering Hakka Tengwar samples...")
