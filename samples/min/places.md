# Places

| Romanization | Hanzi | English | Tengwar | Names |
|---|---|---|---|---|
| tai5 uan5 | 台灣 | Taiwan |  | `{tinco}[a][e/rise]{telco}{vala}[a]{numen}[e/rise]` |
| tai5 pak4 | 台北 | Taipei |  | `{tinco}[a][e/rise]{parma}[a]{calma}` |
| tai5 lam5 | 台南 | Tainan |  | `{tinco}[a][e/rise]{lambe}[a]{malta}[e/rise]` |
| ko1 hiong5 | 高雄 | Kaohsiung |  | `{calma}[o]_{halla}{anna}[o]{noldo}[e/rise]` |
| hok4 kian3 | 福建 | Fujian |  | `{halla}[o]{calma}{calma}{anna}[a]{numen}\..` |
| e7 mng5 | 廈門 | Xiamen |  | `{telco}[e/rise]_[./mid]{malta}{noldo}[e/rise]` |
| tsuan5 tsiu1 | 泉州 | Quanzhou |  | `{tinco_ext}{vala}[a]{numen}[e/rise]{tinco_ext}{anna}[u]_` |
| tsiang1 tsiu1 | 漳州 | Zhangzhou |  | `{tinco_ext}{anna}[a]{noldo}_{tinco_ext}{anna}[u]_` |
| sin1 ka1 pho1 | 新加坡 | Singapore |  | `{thule}[i]{numen}_{calma}[a]_{parma}+asp[o]_` |
| ma2 lai5 | 馬來 | Malaysia |  | `{malta}[a]\{lambe}[a][e/rise]` |
