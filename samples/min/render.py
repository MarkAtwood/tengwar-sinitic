#!/usr/bin/env python3
"""
Render Min (Hokkien) Tengwar samples to SVG.
...
def render_text_to_svg(lines_data, output_path, font_size=48):
    """
    Render lines with Tai-lo, hanzi, tengwar, and romanized names to SVG.

    Args:
        lines_data: list of (tailo, hanzi, tengwar, names) tuples
    ...
    svg = [
        ...
        '<defs><style>',
        f'.tailo {{ font-family: sans-serif; font-size: {int(font_size*0.5)}px; fill: #666; }}',
    ...
    y = 30
    for tailo, hanzi, tengwar, names in lines_data:
        svg.append(f'<text x="20" y="{y}" class="tailo">{tailo}</text>')
        y += row_height
...
def render_sample(name):
    ...
    # Re-generate tengwar from Tai-lo for accurate rendering
    lines_data = []
    ...
    for line in content.split('\n'):
        if '|' in line and not line.startswith('|--'):
            ...
            if len(parts) >= 3:
                ...
                hanzi = parts[2]
                # Skip header rows
                if tailo and hanzi and 'Tai-lo' not in tailo and tailo != '':
                    try:
                        tengwar = convert_text(tailo)
                        names = tengwar_to_names(tengwar)
                        lines_data.append((tailo, hanzi, tengwar, names))
                    except Exception as e:
                        print(f"  Warning: could not convert '{tailo}': {e}")
...
def main():
    # Available samples
    samples = ['tones', 'numbers', 'greetings', 'places', 'nasal_vowels']
    ...
    print("Rendering Min (Hokkien) Tengwar samples...")
