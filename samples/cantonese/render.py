#!/usr/bin/env python3
"""
Render Cantonese Tengwar samples to SVG.
...
def render_text_to_svg(lines_data, output_path, font_size=48):
    """
    Render lines with jyutping, hanzi, tengwar, and romanized names to SVG.

    Args:
        lines_data: list of (jyutping, hanzi, tengwar, names) tuples
    ...
    svg = [
        ...
        '<defs><style>',
        f'.jyutping {{ font-family: sans-serif; font-size: {int(font_size*0.5)}px; fill: #666; }}',
    ...
    y = 30
    for jyutping, hanzi, tengwar, names in lines_data:
        svg.append(f'<text x="20" y="{y}" class="jyutping">{jyutping}</text>')
        y += row_height
...
def parse_sample_markdown(md_path):
    """Parse a Cantonese sample markdown file and extract data."""
    ...
    for line in content.split('\n'):
        if '|' in line and not line.startswith('|--') and 'Jyutping' not in line and 'Tengwar' not in line:
            parts = [p.strip() for p in line.split('|')]
            if len(parts) >= 5:
                # Format: | jyutping | hanzi | tengwar | names |
                # or: | jyutping | hanzi | english | tengwar | names |
                jyutping = parts[1]
                hanzi = parts[2]
                if jyutping and hanzi and not jyutping.startswith('#'):
                    # Find tengwar (PUA characters) and names (backtick-wrapped)
                    tengwar = ''
                    names = ''
                    for p in parts[3:]:
                        if p.startswith('`') and p.endswith('`'):
                            names = p[1:-1]
                        elif re.search(r'[\ue000-\ue0ff]', p) or p.strip() == '':
                            tengwar = p
                        elif '{' in p:
                            names = p

                    if jyutping:
                        lines_data.append((jyutping, hanzi, tengwar, names))
...
def render_sample(name):
    ...
    from tengwar_cantonese import convert_text, tengwar_to_names
    ...
    # Re-generate tengwar from jyutping for accurate rendering
    lines_data = []
    with open(md_path, 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    for line in content.split('\n'):
        if '|' in line and not line.startswith('|--'):
            ...
            if len(parts) >= 3:
                ...
                hanzi = parts[2]
                # Skip header rows
                if jyutping and hanzi and 'Jyutping' not in jyutping and jyutping != '':
                    try:
                        tengwar = convert_text(jyutping)
                        names = tengwar_to_names(tengwar)
                        lines_data.append((jyutping, hanzi, tengwar, names))
                    except Exception as e:
                        print(f"  Warning: could not convert '{jyutping}': {e}")
...
def main():
    # Available samples
    samples = ['tones', 'numbers', 'greetings', 'places', 'poetry']
    ...
    print("Rendering Cantonese Tengwar samples...")
